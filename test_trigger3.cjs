...
async function runTests() {
  ...
  // Set fee to 20 for the tenant
  const { data: tenants } = await supabase.from('tenants').select('id').limit(1);
  const tenantId = tenants[0].id;
  await supabase.from('tenant_settings').update({ home_fee_amount: 20, home_fee_type: 'fixed' }).eq('tenant_id', tenantId);
  ...
  if (!res3.error) {
    const { data: check } = await supabase.from('bookings').select('travel_fee').eq('order_number', 'TEST-MALICIOUS-4').single();
